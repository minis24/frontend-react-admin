...
(function (factory) {
	if (typeof define === 'function' && define.amd) {
		// AMD
		define(['jquery', 'datatables.nets'], function ($) {
	...
	$.extend(TreeGrid.prototype, {
		fnConstruct: function (oInit) {
			...
			// Expand TreeGrid
			dataTable.off('click').on('click', 'td.treegrid-control', function (e) {
				if (!$(this).html()) {
					return;
				}
				...
				//var row = dataTable.row({ parentTr });
				var row = dataTable.row(parentTr._DT_RowIndex);
				var index = row.index();
				var data = row.data();
				...
				if (data.children && data.children.length) {
					...
					data.children.forEach(function (item) {
						...
						//console.log("treegridTd.find('span') >> " + treegridTd.find('span'));
						//treegridTd.find('span').css('marginLeft', '30px');
						//treegridTd.next().css('paddingLeft', '55px');
